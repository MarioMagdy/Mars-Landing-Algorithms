...
vel_vector = np.array([x_velocity, y_velocity,0.0])
...
velocity = np.zeros(3)  # 2D velocity (vx, vy)
position = np.zeros(3)  # 2D position (x, y)
attitude = np.zeros(3)  # Euler angle (yaw in 2D)
