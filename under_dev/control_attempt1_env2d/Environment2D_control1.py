...
# Assuming constant Cd values for different phases 
CD_INITIAL_ENTRY = 0.7
CD_CONTROLLED_DESCENT = 1.2

# Reference area based on lander configuration (replace with calculations)
REFERENCE_AREA_INITIAL_ENTRY = 50.27  # (replace with calculation for heat shield)
REFERENCE_AREA_CONTROLLED_DESCENT = 15.0  # (replace with calculation for wings)
...
# Descent profile example (Desired position, velocity and orientation)
descent_profile = [
    (0, 120000, 6000, -5333, 0),  #Time, Postion, Horizontal velocity, Vertical velocity, Orientation
    (30, 90000, 5500, -4500, 5),
    (60, 60000, 4800, -3500, 10),
]


# PID control constants (adjust these for optimal performance)
Kp_altitude = 1.0
Ki_altitude = 0.1
Kd_altitude = 0.5
Kp_horizontal = 0.8
Ki_horizontal = 0.05
Kd_horizontal = 0.2
previous_error_altitude = 0.0  # Initialize to zero for the first iteration
#integral_error_altitude = 0.0
previous_error_horizontal = 0.0  # Initialize to zero for the first iteration
...
thrust_x = 100.0  # m/s^2 (horizontal thrust)
thrust_y = 100.0  # m/s^2 (vertical thrust)
...
def get_desired_state_from_profile(current_time):
    for i in range(len(descent_profile) - 1):
        if descent_profile[i][0] <= current_time < descent_profile[i + 1][0]:
            # Linear interpolation between profile points
            t0, altitude0, v_horizontal0, v_vertical0, orientation0 = descent_profile[i]
            t1, altitude1, v_horizontal1, v_vertical1, orientation1 = descent_profile[i + 1]
            dt = t1 - t0
            ratio = (current_time - t0) / dt
            desired_altitude = altitude0 + (altitude1 - altitude0) * ratio
            desired_v_horizontal = v_horizontal0 + (v_horizontal1 - v_horizontal0) * ratio
            desired_v_vertical = v_vertical0 + (v_vertical1 - v_vertical0) * ratio
            desired_orientation = orientation0 + (orientation1 - orientation0) * ratio
            return desired_altitude, desired_v_horizontal, desired_v_vertical, desired_orientation
...
def apply_thrust(dt):
    ...
    desired_altitude, desired_v_horizontal, desired_v_vertical, desired_orientation = get_desired_state_from_profile(time_from_start)

    # PID control for altitude
    error_altitude = desired_altitude - y_position
    integral_error_altitude = error_altitude * dt
    integral_error_altitude += integral_error_altitude
    previous_error_altitude = error_altitude
    derivative_error_altitude = (error_altitude - previous_error_altitude) / dt
    vertical_force_pid = Kp_altitude * error_altitude + Ki_altitude * integral_error_altitude + Kd_altitude * derivative_error_altitude
    

    # PID control for horizontal movement 
    error_horizontal = desired_v_horizontal - x_velocity
    integral_error_horizontal = error_horizontal * dt
    integral_error_horizontal += integral_error_horizontal
    previous_error_horizontal = error_horizontal
    derivative_error_horizontal = (error_horizontal - previous_error_horizontal) / dt
    horizontal_force_pid = Kp_horizontal * error_horizontal + Ki_horizontal * integral_error_horizontal + Kd_horizontal * derivative_error_horizontal
    ...
    if fuel_remaining > 0:
        ...
        net_vertical_force = right_wing_vertical_force + left_wing_vertical_force + vertical_force_pid
        net_horizontal_force = top_horizontal_force + bottom_horizontal_force + horizontal_force_pid


        #drag_force = 0.5 * get_mars_atmospheric_density(y_position) * (x_velocity * x_velocity + y_velocity * y_velocity) * calculate_drag_coefficient * REFERENCE_AREA_INITIAL_ENTRY
        # Drag force
        #net_horizontal_force -= drag_force  # Apply drag in the opposite direction of total velocity
...
def handle_thrust_controls(spacecraft_state):
    """
    ...
    # Adjust thruster powers based on spacecraft orientation
    if orientation > 45:  # Example condition for a tilted orientation
        # Apply additional thrust to correct orientation
        top_left_thruster_power += 20.0
        bottom_right_thruster_power += 20.0
    elif orientation < -45:  # Example condition for opposite tilt
        # Apply additional thrust to correct orientation
        top_right_thruster_power += 20.0
...
def draw_environment(screen):
    ...
    mars_texture = pygame.image.load(r'Mars-landing-algorithms\under_dev\control_attempt1_env2d\docs\mars_texture.png').convert_alpha()
    mars_background = pygame.image.load(r'Mars-landing-algorithms\under_dev\control_attempt1_env2d\docs\mars_background.png').convert_alpha()
...
while running:
    ...
    #keys_pressed = pygame.key.get_pressed()
    handle_thrust_controls(spacecraft_state)
    # Increment current time
    time_from_start += dt  # Increment time by timestep dt
