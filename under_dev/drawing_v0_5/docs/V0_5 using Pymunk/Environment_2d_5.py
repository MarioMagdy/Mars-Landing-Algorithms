...
clock = pygame.time.Clock()
space = pymunk.Space()
space.gravity = (0, MARS_GRAVITY)  # Set gravity for the simulation




def apply_thrust(spacecraft_body, dt, max_thrust, control_ori_st, control_pos_st, MARS_GRAVITY, total_fuel_time, fuel_remaining):
    # Assuming thruster powers are attributes of the spacecraft_body for easy access
    left_wing_vertical_force = (spacecraft_body.left_wing_up_thruster_power - spacecraft_body.left_wing_down_thruster_power) / 100.0 * max_thrust
    right_wing_vertical_force = (spacecraft_body.right_wing_up_thruster_power - spacecraft_body.right_wing_down_thruster_power) / 100.0 * max_thrust
    top_horizontal_force = (spacecraft_body.top_right_thruster_power - spacecraft_body.top_left_thruster_power) / 100.0 * max_thrust
    bottom_horizontal_force = (spacecraft_body.bottom_right_thruster_power - spacecraft_body.bottom_left_thruster_power) / 100.0 * max_thrust
...
def create_spacecraft_body(space, mass, moment, points):
...
while running:
    ...
    # Handle user input for thrust controls
    keys_pressed = pygame.key.get_pressed()
    # ... (handle thrust controls as before, but apply forces to the Pymunk body)

    # Step the simulation
    dt = clock.tick(60) / slower
    space.step(dt)
...
while running:
    ...
    air = air_more_realistic_f.maintain_air(c= c,space=screen,air=air,pos=[x_position//SCALE,SCREEN_HEIGHT+(-y_position-10)//SCALE])
