...
### v4.1 changes:
def draw_spacecraft(screen, x, y, angle,spacecraft_points):
    ...
    rotated_points = []
    for point in spacecraft_points:
...
while running:
    ...
    air = air_more_realistic_f.maintain_air(c,air,pos=[x_position//SCALE,SCREEN_HEIGHT+(-y_position-10)//SCALE])
    air_more_realistic_f.update_air(air,c)
    ...
    for i in air :
        air_more_realistic_f.check_collision_spacecraft(air_particle=i,spacecraft_points=spacecraft_points)
    

    draw_spacecraft(screen, spacecraft_x, spacecraft_y,spacecraft_orientation,spacecraft_points)
