...
x = np.array([12000, 0, 0, 2000, 0, 0, 0, 0, 0, 0.1, 0.1, 0.1, 1, 0, 0, 0])  # state vector
sigma = 0.1  # Bank angle in radians
...
b = 10  # Example wingspan in meters
...
m = 2000  # Example mass of the lander in kg
...
Q_lat = np.diag([1/max_roll_rate_deviation**2, 1/max_yaw_rate_deviation**2, 
                 1/max_sideslip_angle_deviation**2, 1/max_bank_angle_deviation**2])
R_lat = np.diag([1/max_aileron_deflection**2, 1/max_rudder_deflection**2, 
                 1/max_RCS_torque_x**2, 1/max_RCS_torque_z**2])

# Compute the LQR gain matrices
...
# Calculate the required forces and torques
force_long = u_long[0]  # Longitudinal force from elevator deflection
torque_long = u_long[1]  # Longitudinal torque from RCS y-axis

force_lat = np.array([0, u_lat[0], u_lat[1]])  # Lateral forces
torque_lat = np.array([u_lat[2], 0, u_lat[3]])  # Lateral torques from RCS x and z axes

# Combine the forces and torques
force = np.array([force_long, force_lat[1], force_lat[2]])
torque = np.array([torque_lat[0], torque_long, torque_lat[2]])

print("Force", force)
print("Torque", torque)


# Solve for the thruster commands using quadratic programming
def thruster_commands(force, torque, P_new, V):
    ...
    def objective(thruster_cmd):
        return np.sum(thruster_cmd**2)
    
    constraints = [{'type': 'eq', 'fun': lambda thruster_cmd: D @ thruster_cmd - FT}]
    
    bounds = [(0, None) for _ in range(4)]
    
    result = minimize(objective, np.zeros(4), constraints=constraints, bounds=bounds)
    
    return result.x

thrusters_cmd = thruster_commands(force, torque, P_new, V)
